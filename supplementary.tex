...
\large Appendices A--C: Transcripts, Onboarding, and VLM Session}
...
\textit{UnMask:} Let's think through the anatomy carefully. What is the primary muscle responsible for shoulder abduction beyond 15 degrees, and which nerve innervates it?\\
...
\textit{UnMask:} What do you know about the anatomy of the carpal tunnel --- what structures pass through it, and which one would be most vulnerable to compression if the tunnel narrows?\\
...
\noindent\textbf{PCR mode transition at Turn 3:}
Mastery starts at 0.20 (\texttt{context\_only}, $m < 0.40$).
After each correct response, $m' = m + 0.15(1-m)$.
Two correct turns advance mastery from 0.20 $\to$ 0.33 $\to$ 0.43,
crossing the 0.40 threshold and enabling \texttt{prerequisite\_first} mode.
Transcripts where Turn 2 is also substantially correct (e.g., Transcripts 1, 3, 4)
show this transition; the LLM judge verified each transition is consistent with
the student's demonstrated mastery.
...
\noindent\textbf{UnMask reply with inline diagram:}\\
\textit{``That's a tricky one to visualize! Here is the brachial plexus
diagram for reference. [\texttt{DiagramCard} renders animated anatomy HTML inline.] Look at where C5--C6
converge: what structure do they form at that junction?''}

\subsection*{B.3 Session Wrapup Summary (Structured Output)}

At wrapup ($t \geq 840$s), Mercury-2 structured output produces
...
The image is base64-encoded and POSTed to
\texttt{POST /api/sessions/\{id\}/image}; the backend runs a two-step VLM pipeline.

\noindent\textbf{System action} (two-step pipeline, \texttt{api.py}):
\begin{lstlisting}
# Step 1: Gemini 2.0 Flash Lite identifies structure
#   (internal only, max 50 tokens, name only)
vision_model = "google/gemini-2.0-flash-lite"
identified = "brachial plexus"  # internal

# Step 2: Mercury-2 generates Socratic question
#   system: "Student uploaded image of brachial plexus.
#   Ask ONE Socratic question. Do NOT name the structure."

...
\node[sub, below=0.28cm of sout, fill=yellow!18] (llm)
  {GPT-4o (tutor/assess/wrap)\\GPT-4o-mini (supervisor)};
\draw[darr] (gen)  -- (sout);
\draw[darr] (sout) -- (llm);
...
\end{tikzpicture}
\caption{End-to-end query flow through UnMask. The student message enters the LangGraph state machine via a Next.js frontend (SSE stream); the Supervisor (GPT-4o-mini rule/LLM hybrid) routes tutoring/assessment turns through the Retrieval Planner (PCR-gated hybrid RAG + CRAG), then the Socratic Generator (GPT-4o via OpenRouter, structured output with hidden \texttt{InternalAnalysis}), then the Pedagogy Agent (mastery update, concept DAG, mistake log). Rapport and wrapup bypass retrieval.}
...
\midrule
Supervisor & GPT-4o-mini & Phase routing (LLM branch) \\
Tutoring / Assessment & GPT-4o & Structured Socratic output \\
Wrapup summary & GPT-4o & \texttt{SessionSummary} + YouTube recs \\
Embeddings & Gemini Emb.\ 2 (3072d) & Dense retrieval \\
CRAG relevance & GPT-4o-mini & Binary chunk grading \\
Image verification & Gemini 2.0 Flash Lite (vision) & Web diagram filtering \\
Student image VLM & Gemini 2.0 Flash Lite & Upload identification \\
\bottomrule
\end{tabular}
\caption{Models used per component. All LLM calls via OpenRouter.}
\label{tab:models}
\end{table}
...
Progress tab.
\textbf{Cost:} GPT-4o for Tutoring, Assessment, and Wrapup summary;
GPT-4o-mini for Supervisor and CRAG grading (${\sim}\$0.08$--$0.10$/session).

...
educational tutoring), dual-layer structured output masking,
\textbf{session mistake memory} (proactive revisit at 8 min using stored misconceptions),
and a \textbf{multimodal Next.js frontend} with VLM-based image upload, animated anatomy
diagrams, and session YouTube recommendations.
We evaluated on 30 QA triples with a four-variant ablation: Hit Rate 0.900,
Leak Rate 0.000, Socratic Purity 4.87/5, 100\% adversarial hold rate.
...
RAGAS Faithfulness (0.838) shows a metric mismatch: RAGAS penalizes
Socratic questions that make no factual claims, which is expected here.
Live demo: \url{https://huggingface.co/spaces/Gustav-Proxi/UnmaskTutor}
\end{abstract}
...
Table~\ref{tab:models} lists all models used. All LLM calls route through
OpenRouter (\texttt{openrouter.ai/api/v1}) so the base URL is the only config
change needed to swap providers.
...
Students interact via a Next.js 14 App Router frontend (live at
\url{https://huggingface.co/spaces/Gustav-Proxi/UnmaskTutor}).
The UI streams responses via SSE from the FastAPI backend.
A right-hand inspector panel (instructor-visible) displays per-turn: PCR
retrieval mode, mastery state per concept, and misconception log, directly
satisfying the requirement that the UI show how chunks are being processed.
A Chainlit interface (\texttt{app.py}) is retained for backward compatibility.
...
every concept (brachial plexus, peripheral nerves, rotator cuff, spinal cord);
for concepts without a local file, a DuckDuckGo image search queries trusted
anatomy domains (Wikipedia, Radiopaedia, Kenhub), and Gemini 2.0 Flash Lite (vision)
verifies the top-3 results before displaying the first verified URL.
...
To our knowledge, this is the first application of CRAG in educational tutoring.
...
If the student asks for an explicit explanation (``can you explain'',
``walk me through'', etc.) or hits 4 consecutive incorrect answers,
\texttt{break\_socratic=True} swaps to a \texttt{\_REVEAL\_SYSTEM} prompt that
instructs the model to explain directly rather than question again.

\textbf{Wrapup summary and YouTube recommendations.}
At Wrapup, Mercury-2 structured output produces a \texttt{SessionSummary}:
per-concept mastery status, misconceptions from \texttt{mistake\_log},
actionable study tips, and 2--4 \texttt{YouTubeResource} objects for the
weakest concepts, rendered as clickable search cards in the frontend
Progress tab.
\textbf{Cost:} Mercury-2 for all LLM components (Supervisor, Tutoring, Assessment, CRAG grading, and Wrapup summary); Gemini 2.0 Flash Lite for vision tasks (${\sim}\$0.01$--$0.02$/session).
...
When a student uploads an anatomical image via the Next.js frontend,
UnMask invokes Gemini 2.0 Flash Lite (vision) via OpenRouter in a two-step pipeline:
first identify the structure internally, then generate a Socratic question
...
name only), keeping the identification internal while the tutor model generates
the Socratic question without ever having the structure name in the user-visible prompt.
...
\paragraph{Corpus.}
OpenStax Anatomy \& Physiology 2e \cite{openstax2019}, Chapters 11 and 13--16, covering NBCOT
...
This is the same failure mode Dinucu-Jianu et al.\ \cite{tutorrl2025} found: sufficiently capable models'
...
Socratic Purity (4.87/5, LLM-as-Judge) is the appropriate groundedness proxy.
